...
function WhatsAppFloatingButton({ bubbleLine1 = "¿Consultas y reservas?", bubbleLine2 = "¡Hablemos!", }) {
    ...
    return (
        <div className="fixed bottom-4 right-5.5 flex flex-row gap-2 z-50 items-end">
            <div className={`chat-bubble bg-[#F5F7F9] p-3 rounded-lg ${bubbleTransitionClass}`}>
                <h3 className="text-defect text-[.8rem]">{bubbleLine1}</h3>
                <h3 className="text-defect text-[.8rem] font-bold text-[#3E6143]">{bubbleLine2}</h3>
            </div>

            <div ref={buttonRef} className={`whatsapp-btn ${!hasInteracted ? "whatsapp-pulse" : ""} bg-[#2DB742] h-13 w-13 z-70 flex items-center justify-center rounded-full relative cursor-pointer`} onClick={handleClickWhatsapp} role="button" aria-expanded={openMenuWhatsapp} aria-label="Abrir opciones de WhatsApp" tabIndex={0} onKeyDown={(e) => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); handleClickWhatsapp(); } }} >
                ...
                <img src={crossIcon} alt="" className={`w-8 h-8 invert transition-all duration-300 absolute ${ openMenuWhatsapp ? "rotate-0 opacity-100" : "-rotate-180 opacity-0" }`} />
            </div>

            <div ref={menuRef} className={`wa-menu bg-white z-60 right-7.5 w-[85%] md:w-90 fixed bottom-20 ${menuTransitionClass}`} >
                ...
                <div className="py-2">
                    {WHATSAPP_OPTIONS.map((opt) => (
                        <a key={opt.title} href={opt.href} target="_blank" rel="noopener noreferrer" className={`wa-option wa-option--${opt.variant} flex flex-row items-center gap-3 px-5 py-3.5 cursor-pointer ${ openMenuWhatsapp ? "fade-down" : "" }`} style={{ animationDelay: opt.delay }} >
                            ...
                            <svg className="wa-chevron w-4 h-4 flex-shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" >
